---
name: debt
description: Audit the repo for `on:` routing-decision markers and print a ledger grouped by file, flagging entries that name no upgrade trigger. Use to review accumulated deliberate-shortcut debt left by /impl. Read-only; never edits or removes markers.
---

# /debt

Surface the deliberate routing shortcuts the workflow has taken. A `// on:` marker (`# on:` for Python/shell) records a borderline classification call /impl made on purpose — routing simple where medium was plausible, treating stakes as normal where high was arguable, skipping a review pass on a borderline change. Each marker should name an upgrade trigger: the condition under which the shortcut stops being safe. `/debt` collects them so the debt stays visible instead of silently compounding.
...
1. Search the repo for markers (ripgrep preferred; fall back to grep):
   ...
   ```
   rg -n -- 'on:|dD:' .        # or: grep -rn -E 'on:|dD:' .
   ```

   Match both `// on:` and `# on:` forms, and accept legacy `// dD:` and `# dD:` forms.
...
3. For each surviving match, extract: file path, line number, and the marker text after `on:` or legacy `dD:`.
